...
describe('create application', () => {
    ...
    test('should create stanford user application with in-state', async () => {
        ...
        const application: IApplication = await createApplication({ email: "test@stanford.edu", sub: "123124", "custom:location": "CA" });
        expect(application.type).toEqual("stanford");
    });

    test('should create user application in-state', async () => {
        const application: IApplication = await createApplication({ email: "test@berkeley.edu", sub: "123125", "custom:location": "CA" });
        expect(application.type).toEqual("is");

    });

    test('should create user application out-of-state', async () => {
        const application: IApplication = await createApplication({ email: "test@gatech.edu", sub: "123126", "custom:location": "GA" });
