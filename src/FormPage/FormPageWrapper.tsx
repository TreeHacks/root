...
import {
  ...
  applicationDisplayFieldsStanford,
} from "../constants";
...
class FormPageWrapper extends React.Component<
  ...
  { showSavedAlert: boolean; showModal: boolean }
> {
  constructor(props) {
    super(props);
    this.state = {
  ...
  onSubmit(submit) {
    ...
    (get(props, "profile.status") !== "incomplete"
  ...
  render() {
    if (
      !this.props.formData ||
      this.props.formName !== this.props.incomingFormName
    ) {
      return <Loading />;
    }
    const props = this.props;
    ...
    if (get(schema, "properties.section1")) {
      schema.properties.section1.custom_description = (
        <span>
          We’re so excited that you want to come to TreeHacks 2023!{" "}
          <b>
    ...
    if (get(schema, "properties.accept_terms")) {
      schema.properties.accept_terms.title = (
        <span>
    ...
    if (get(schema, "properties.accept_share")) {
      schema.properties.accept_share.title = (
        <span>
          I have read and agree to the{" "}
          <a
            className="form-link"
    ...
    const alertMessage = submitted
      ...
      : hasDeadlinePassed
      ? "Sorry, the application window has closed."
      : this.state.showSavedAlert
      ? `Your application progress has been saved. Make sure you finalize and submit before the deadline.`
      : null;

    const unsavedChangesWarning =
        'You have unsaved changes to your application. Are you sure you want to leave? You can save your application to continue later by clicking "Save for later" at the bottom of the page.',
    ...
    return (
      ...
      >
        <Beforeunload
          onBeforeunload={(e) =>
            showUnsavedWarning ? unsavedChangesWarning : true
          }
        />
        ...
        {!hasDeadlinePassed ? (
          <FormPage
            ...
            onChange={(e) => {
              const userEdited =
                JSON.stringify(e.formData) !== JSON.stringify(props.formData);
        ...
        <Modal open={this.state.showModal} onClose={() => this.onCloseModal()}>
          <div className="my-1 modal-window">
            <div className="my-4">
              Are you sure? You won't be able to edit your application once you
              submit.
            </div>
            <div className="row">
              <div className="col-6 text-left">
                <div
                  className="mt-2 btn btn-modal inverted"
