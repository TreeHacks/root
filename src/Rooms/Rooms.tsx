...
export class Rooms extends React.Component<IRoomsProps, IRoomsState> {
  ...
  _updateFromApi({ current_room, rooms }) {
    if (current_room) {
      current_room.expiry = new Date(current_room.expiry);
    }
  ...
  onCancel() {
    if (
      !window.confirm(
        "just making sure—are you sure you'd like to drop this room? thanks for being so considerate and giving other hackers a chance!"
      )
    ) {
      return;
    }
    ...
    return (
      ...
      >
        <div className="treehacks-alert" style={{ maxWidth: "750px" }}>
          <div>
            Sometimes, you need private space for your team to work. Now you can
            have it! So that we can keep offering dedicated room space, make
            sure you follow the rules:
            <ul className="rules">
              <li>
                Only reserve a room if your team <u>truly needs one</u>.
              </li>
              <li>Don't reserve the same room multiple times in a row.</li>
              <li>
                Room reservations last for <u>one hour</u> and are
                first-click-first-serve. If you finish early, drop your
                reservation so that others can use the room!
              </li>
              <li>
                Don't "game the system" by having different team members reserve
                in sequence. Not only is this morally reprehensible, we'll also
                be checking during judging{" "}
                <code style={{ color: "#212529" }}>;)</code>
              </li>
            </ul>
          </div>
            ...
            {currentRoom ? (
              <div className="rooms">
                <div>
                  ...
                  <div>
                    expires in{" "}
                    {this._formatTimeDelta((+currentRoom.expiry - now) / 1000)},
                    or&nbsp;
                ...
                return (
                  <div>
                    <div>
                      ...
                      {next_unavailable ? (
                        <small>
                          "{next_unavailable.label}" begins here in{" "}
                          {this._formatTimeDelta(
                            (+new Date(next_unavailable.start) - now) / 1000
                          )}
                        </small>
                      ) : null}
                    </div>
                    <div>
                      {error ? (
                        `${error.replace(
                          /%EXPIRY%/g,
                          moment(expiry).format("h:mma")
                        )}`
                      ) : currentRoom ? (
                        `you already have a room!`
                      ) : !expiry || delta <= 0 ? (
                        <a href="#" onClick={this.onReserve.bind(this, id)}>
                          reserve now!
                        </a>
                      ) : (
