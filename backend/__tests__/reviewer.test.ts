...
import {
  ...
  applicationReviewDisplayFieldsNoSection,
  TYPE,
} from "../constants";
  ...
  test("review application gets the right fields", async () => {
    ...
    return request(app)
      ...
      .then((e) => {
        ...
        expect(Object.keys(e.body.forms.application_info).sort()).toEqual(
          applicationReviewDisplayFieldsNoSection.sort()
        );
      });
  });
    ...
    await Application.insertMany([
      {
        ...
        type: TYPE.OUT_OF_STATE,
        status: STATUS.SUBMITTED,
        reviews: [{ reader: { email: "reviewer1@treehacks" } }],
      },
      {
        user: { id: "applicationOos2" },
        year: HACKATHON_YEAR_STRING,
        type: TYPE.OUT_OF_STATE,
        status: STATUS.SUBMITTED,
        reviews: [
          { reader: { email: "reviewer1@treehacks" } },
          { reader: { email: "reviewer2@treehacks" } },
        ],
      },
    ]);
    await request(app)
      ...
      .expect(200)
      .then((e) => {
        expect(e.body.sort()).toEqual([
          { _id: "reviewer1@treehacks", count: 2 },
          { _id: "reviewer2@treehacks", count: 1 },
        ]);
      });
  });
});
