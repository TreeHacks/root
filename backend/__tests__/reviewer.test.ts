...
import { STATUS, sponsorApplicationDisplayFields, applicationReviewDisplayFields, TYPE } from '../constants';
...
describe('review next application', () => {
    test('review application gets the right fields', async () => {
        ...
        return request(app)
            ...
            .expect(200)
            .then(e => {
                expect(Object.keys(e.body)).toEqual(["_id", "forms"]);
    ...
    test('review application gets oos before is', async () => {
        await Application.insertMany([
            { _id: "applicationOos", type: TYPE.OUT_OF_STATE, status: STATUS.SUBMITTED },
            ...Array(100).fill({ type: TYPE.IN_STATE, STATUS: STATUS.SUBMITTED })
        ]);
        for (let i = 0; i < 10; i++) {
            await request(app)
                .get("/review/next_application")
                ...
                .expect(200)
                .then(e => {
                    expect(e.body._id).toEqual("applicationOos");
                });
        }
    });
    test('review application gets is application', async () => {
        ...
        await request(app)
            ...
            .expect(200)
            .then(e => {
                expect(e.body._id).toEqual("applicationIs");
            });
    });
    ...
    test('review application does not get application already reviewed by current user', async () => {
        await new Application({
            _id: "applicationIs", type: TYPE.OUT_OF_STATE, status: STATUS.SUBMITTED,
            reviews: [{
                reader: {
...
describe('rate applications', () => {
    test('rate an application', async () => {
        ...
        let application = (await Application.findById('applicationToReview'))!.toObject();
...
describe('review leaderboard', () => {
    test('simple leaderboard test', async () => {
        await Application.insertMany([
            { _id: "applicationOos", type: TYPE.OUT_OF_STATE, status: STATUS.SUBMITTED, reviews: [{reader: {email: "reviewer1@treehacks"}}] },
...
describe('review stats', () => {
    test('simple stats test', async () => {
        await Application.insertMany([
            { _id: "applicationOos", type: TYPE.OUT_OF_STATE, status: STATUS.SUBMITTED, reviews: [] },
            { _id: "applicationOos2", type: TYPE.OUT_OF_STATE, status: STATUS.SUBMITTED, reviews: [{}, {}, {}] },
            { _id: "applicationIss", type: TYPE.IN_STATE, status: STATUS.SUBMITTED, reviews: [{}] },
