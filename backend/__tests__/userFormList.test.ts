...
import {
  STATUS,
  sponsorApplicationDisplayFieldsNoSection,
  HACKATHON_YEAR_STRING,
} from "../constants";
...
let docs = [
  ...
  {
    ..._doc,
    user: { email: "test@treehacks", id: "applicant-optout-confirmed" },
    sponsor_optout: true,
    status: STATUS.ADMISSION_CONFIRMED,
  },
  {
    ..._doc,
    user: { email: "test@treehacks", id: "applicant-confirmed-3" },
    status: STATUS.ADMISSION_CONFIRMED,
  },
  {
    ..._doc,
    user: { email: "test@treehacks", id: "applicant-confirmed" },
    forms: {
      ..._doc.forms,
      meet_info: { showProfile: false },
    },
    status: STATUS.ADMISSION_CONFIRMED,
  },
  {
    ..._doc,
    user: { email: "test@treehacks", id: "applicant-confirmed-2" },
    status: STATUS.ADMISSION_CONFIRMED,
    forms: {
      ..._doc.forms,
      meet_info: undefined,
  ...
  {
    ..._doc,
    user: { email: "test@treehacks", id: "applicant-submitted" },
    status: STATUS.SUBMITTED,
  },
  {
    ..._doc,
    user: { email: "test@treehacks", id: "applicant-name-thomas" },
    forms: {
      ..._doc.forms,
      application_info: {
  ...
  {
    ..._doc,
    user: { email: "test@treehacks", id: "applicant-name-tracey" },
    forms: {
      ..._doc.forms,
      application_info: {
        ..._doc.forms.application_info,
  ...
  test("view only application confirmed forms with no opt out", () => {
    return request(app)
      ...
      .then((e) => {
        ...
        expect(e.body.results.map((item) => item.user.id).sort()).toEqual(
          [
            "applicant-confirmed",
            "applicant-confirmed-2",
            "applicant-confirmed-3",
          ].sort()
        );
        for (let result of e.body.results) {
          expect(Object.keys(result.forms.application_info).sort()).toEqual(
            sponsorApplicationDisplayFieldsNoSection.sort()
          );
        }
      });
  });
...
describe("user form list by admin", () => {
  test("view all forms", () => {
    return request(app)
      ...
      .then((e) => {
        expect(
          e.body.results.map((item) => omit(item, ["__v", "_id"])).sort()
        ).toEqual(docs.sort());
      });
  });
  ...
  test("filter by name (partial filter)", () => {
    return request(app)
        ...
        "/api/users?" +
          queryString.stringify({
            filter: JSON.stringify({
              "forms.application_info.first_name": "t",
            }),
          })
      )
      .set({ Authorization: "admin" })
      ...
      .then((e) => {
        ...
        expect(
          e.body.results
            .map((item) => item.forms.application_info.first_name)
            .sort()
        ).toEqual(["thomas", "tracey"]);
      });
  });
    ...
    return request(app)
      .get(
        "/api/users?" +
          queryString.stringify({
            filter: JSON.stringify({
              "forms.application_info.first_name": "a",
            }),
          })
      )
      .set({ Authorization: "admin" })
  ...
  test("filter by name (partial filter) and sort asc", () => {
    return request(app)
      ...
      .then((e) => {
        expect(e.body.results.length).toEqual(2);
        expect(
          e.body.results.map((item) => item.forms.application_info.first_name)
        ).toEqual(["thomas", "tracey"]);
      });
  });
